...
func TestImplementation_DeployStatus(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
	...
	}{
		{
			...
			solutionServiceMock: func(mc *minimock.Controller) service.SolutionService {
				...
				mock.DeployStatusMock.Expect(ctx, 1).Return("running", "", nil)
				return mock
			},
		},
		{
			...
			solutionServiceMock: func(mc *minimock.Controller) service.SolutionService {
				mock := serviceMocks.NewSolutionServiceMock(mc)
				mock.DeployStatusMock.Expect(ctx, 1).Return("", "", serviceErr)
				return mock
			},
		},
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			newID, err := api.GetDeployStatus(tt.args.ctx, tt.args.req)
			if err != nil {
